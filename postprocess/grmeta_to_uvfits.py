...
def load_correlations(r, path, nfft):
    size = (path / (r + f'power_0')).stat().st_size
    N = size // (4 * nfft)  # number of timestamps (4 is sizeof(float32))
...
def main():
    ...
    for scan in scans:
        ...
        baselines = ['-'.join([ants[0], ants[0]]), baseline,
                     '-'.join([ants[1], ants[1]])]
        x = load_correlations(scan, input_path, nfft)
        ...
        uvfits.export_uvfits(
            t0, T, x, uvw, source_name, source, freq, samp_rate / nfft,
            baselines, ant_coordinates, args.t_avg, args.f_avg, output_path)
